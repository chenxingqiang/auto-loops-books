...
> **全书文风规范（强制执行）：** 见 [`books/WRITING_STYLE.md`](books/WRITING_STYLE.md) — 工程师叙事型硬核干货；对标第1章定稿口吻。loops 每轮 `agent_tasks` 均引用该文件。
>
> **事实核验门禁（强制执行）：** 凡事实描述、数据举例须 **Web 检索反复验证** 后方可入稿，并记录可靠链接 → [`books/FACT_VERIFICATION.md`](books/FACT_VERIFICATION.md)（`books/research/<章>/verified_facts.jsonl`）。
