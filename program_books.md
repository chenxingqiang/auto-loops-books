...
**Monorepo note:** Stage only files touched this session: `book_content.md`, `book_prepare.py` (when OUTLINE changed), `books/chapters/*.tex`, `books/book.bib`, `books/main.tex`, `books/research/<id>/`, and `book_results.tsv`. Never use blind `git add -A`.
