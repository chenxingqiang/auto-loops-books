...
**Monorepo note:** Stage only files touched this session: `AI Compiler Performance Engineering.md`, `book_prepare.py` (when OUTLINE changed), `books/chapters/*.tex`, `books/book.bib`, `books/main.tex`, `books/research/<id>/`, and `book_results.tsv`. Never use blind `git add -A`.
...
3. **Read in-scope files**:
   - `books/WRITING_STYLE.md` — **mandatory** unified voice (工程师叙事型硬核干货); gold standard = ch01.
   - `books/FACT_VERIFICATION.md` — **mandatory** fact gate: web-verify numeric claims & examples; log URLs in `verified_facts.jsonl`.
   - `AI Compiler Performance Engineering.md` — full outline and style requirements (Chinese spec; output is English).
   ...
   - `books/` — LaTeX template (Deep Learning book style). Mutable content lives in `books/chapters/*.tex`.
...
- Edit `AI Compiler Performance Engineering.md` — 全书目录、模块划分、章节 bullet、写作意图（中文 spec，可增删改章节与小节）。
...
- Edit `books/chapters/*.tex` — chapter prose, `\section`/`\subsection` structure, citations, tables.
...
- Create new `books/chapters/chXX_*.tex` when OUTLINE gains a chapter (stub → research → expand).
...
| `AI Compiler Performance Engineering.md` | `#### 第N章` bullets + Chinese→English term mapping |
| `books/chapters/<file>.tex` | `\section`, `\index`, `\newterm`, high-frequency body terms |
...
Any **factual description, measurement, vendor spec, or worked example** in `books/chapters/*.tex` must be **web-verified** before it ships. Training-data recall is not evidence.
...
Full protocol: [`books/FACT_VERIFICATION.md`](books/FACT_VERIFICATION.md). `book-loop step` adds `agent_tasks` for missing logs and uncited numeric paragraphs.
...
4. `--render` missing ids; paste snippets under the matching `\section{}` in `books/chapters/*.tex`.
...
| Layer | File | Role |
|-------|------|------|
| 1 — Spec | `AI Compiler Performance Engineering.md` | 人类可读的全书目录与章节意图（source of intent） |
| 2 — Rubric | `book_prepare.py` → `OUTLINE` | 评测用章节 id、文件名、section 覆盖 regex |
| 3 — Book | `books/main.tex` + `books/chapters/*.tex` | 可编译 PDF；`\tableofcontents` 由 LaTeX 自动生成 |
...
1. **Spec** — update `AI Compiler Performance Engineering.md`（`#### 第N章`、bullets、模块表）
...
# books/chapters/ch04_xxx.tex — \chapter{English Title} + \section stubs matching SectionSpec labels
...
3. **Complete `agent_tasks`** from `loop_state.json` (LLM-only):
   ...
   - Edit `books/chapters/*.tex` (and 目录/OUTLINE/main.tex if needed)
...
`book_prepare.py --list` and `AI Compiler Performance Engineering.md` are the live sources — the table below is a **snapshot**, not a freeze:
...
- **Primary manuscript:** English (`books/chapters/*.tex`).
- **Outline spec:** `AI Compiler Performance Engineering.md` is the source of truth for 目录与写作意图; keep it in sync with `OUTLINE` and LaTeX structure.
